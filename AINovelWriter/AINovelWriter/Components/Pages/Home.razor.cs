...
namespace AINovelWriter.Components.Pages;
...
public partial class Home
{
    ...
    [Inject]
    private INovelWriter NovelWriterService { get; set; } = default!;
    ...
    private async void Submit(NovelWriter novelWriter)
    {
        ...
        await Task.Delay(1);
        await foreach (var token in NovelWriterService.WriteNovel(novelWriter.Outline, novelWriter.AIModel, ctoken))
        {
            _text += token;
    ...
    private async void SubmitIdea(NovelIdea novelIdea)
    {
        ...
        _novelOutline = await NovelWriterService.GenerateNovelIdea(novelIdea.NovelGenre);
    ...
    private async void SubmitOutline(NovelOutline novelOutline)
    {
        ...
        StateHasChanged();
        await Task.Delay(1);
        _novelWriter.Outline = await NovelWriterService.CreateNovelOutline(novelOutline.Theme, novelOutline.Characters, novelOutline.PlotEvents, novelOutline.Title, novelOutline.ChapterCount, novelOutline.AIModel);
        _isBusy = false;
        _showOutline = true;
        StateHasChanged();
    }
